...
class HomePage extends React.Component {
	...
	componentDidMount() {
		window.document.addEventListener('doneQuestion', this.handleFrameDoneQuestion.bind(this), false);
	...
	handleFrameDoneQuestion(e) {
		...
		if (e.detail.isCorrect) {
			...
			this.setState({ current_score: new_score, showSuccess: showSuccess, score:this.state.score+1,
				max_score:max_score })
		}
	...
	render() {
		...
		var successFig = "./src/assets/wizard"+Math.floor((Math.random() * 6) + 1)+'.gif';
		...
		return (
			<div className="App" width={Math.min(640000, this.state.width)} //{Math.floor(this.state.width * 0.8)}
			height={Math.min(550, this.state.height)} >
				<div>
					<table style={{ "width": "100%", padding: "10px" }}>
						<tbody>
							<tr>
								<td size="40%" style={{ padding: "7px" }}><Link to="/login" style={{ padding: "7px" }} >Logout</Link></td>
								<td align='left' style={{ padding: "7px" }}> <CircularProgressBar
									strokeWidth="5"
									sqSize="50"
									text={this.state.score}
									percentage={Math.round(100 * this.state.current_score / this.state.max_score)} />
								</td>
								<td align='right'><img onClick={this.goHome} width={"32px"} src={"https://upload.wikimedia.org/wikipedia/commons/thumb/e/e1/YouTube_play_buttom_icon_%282013-2017%29.svg/32px-YouTube_play_buttom_icon_%282013-2017%29.svg.png"} />
								</td>
							</tr>
						</tbody>
					</table>
				...
				<iframe src={"./public/" + this.state.game_id + "/index.html?questions=" + encodeURIComponent(this.state.questionsString)}
					title="game" ref={(f) => this.ifr = f}
					frameBorder={'0'}
					width={Math.min(640000, this.state.width)} height="550"
					style={style}></iframe>


				{this.state.showSuccess? <div className="overlayImage" style={overlay_style}></div> : null}
