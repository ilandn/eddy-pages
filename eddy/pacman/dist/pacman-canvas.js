			...
			if ((this.stuckX == 0) && (this.stuckY == 0) && this.frozen == false) {
				...
				if ((field === "pill") || (field === "powerpill")) {
					...
					if (
						...
						if (field === "powerpill") {
							var pp = 0;
							if (this.posX > game.width / 2) {
								pp = pp + 1;
							}
							if (this.posY > game.height / 2) {
								pp = pp + 2;
							}
							if (questions.checkAnswerNum(pp)) {
		...
		$.each(game.map.posY, function (i, item) {
			...
			$.each(this.posX, function () {
				if (this.type == "pill") {
					...
					//context.arc(game.toPixelPos(this.col-1)+pacman.radius,game.toPixelPos(dotPosY-1)+pacman.radius,game.powerpillSizeCurrent*1.5,0*Math.PI,2*Math.PI);
					//context.moveTo(game.toPixelPos(this.col-1), game.toPixelPos(dotPosY-1));
					...
					if (answers[answerIdx].indexOf('http') >= 0) {
						if (game.answersImages[answerIdx]) {
							let imageObj = game.answersImages[answerIdx];
							let height = 40;
							let newW = Math.floor(height / imageObj.naturalHeight * imageObj.naturalWidth);
							ctx2.drawImage(imageObj, game.toPixelPos(this.col - 1) , game.toPixelPos(dotPosY - 1),
							pacman.radius*2, pacman.radius*2);
						}
						else {
							var imageObj = new Image();

							imageObj.src = answers[answerIdx];							
							game.answersImages[answerIdx] = imageObj;
							let height = 40;
							let newW = Math.floor(height / imageObj.naturalHeight * imageObj.naturalWidth);
							ctx2.drawImage(imageObj, game.toPixelPos(this.col - 1) , game.toPixelPos(dotPosY - 1),
							pacman.radius*2, pacman.radius*2);
						...
						context.fillText(answers[answerIdx], game.toPixelPos(this.col - 1) + pacman.radius, game.toPixelPos(dotPosY - 1) + pacman.radius);
					}
