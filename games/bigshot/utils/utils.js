function initQuestions(questionUI) {
	...
	window.parent.EddySdk.gameLoaded("test");
	if (!questionUI) {
...
function Questions(questionUI) {
	this.eddy_sdk = window.parent.EddySdk;
	...
	this.checkAnswerNum = function (answerNum) {
		if (this.currentQuestion)
			return this.checkAnswer(this.currentQuestion.answers()[answerNum]);
		else
			return false;
	}
	...
	this.checkAnswer = function (answer) {
		...
		let answerRight = answer.isCorrect();
		// var answerMsg = {
	...
	this.newQuestion = function () {
		...
		if (!this.currentQuestion){
			var msg = JSON.parse(('{"name":"QUESTIONS_COMPLETED","data":{}}'));
		...
		if (this.currentQuestion.answers().length > 1){
			...
			this.currentQuestion.type = 'TYPING';
		}
		this.currentQuestion.data = {'answers': this.currentQuestion.answers()};
		if (this.currentQuestion.type == 'MULTIPLE_CHOICE') {
			for (idx = 0;idx < this.currentQuestion.data.answers[idx].length; idx++){
				if (this.currentQuestion.data.answers.isCorrect()){
					this.currentRightAnswerIdx = idx;
					break;
				}
			}
			this.currentAnswers = this.currentQuestion.data.answers.map(x => standartize(x.text()));
		}
		if (this.questionUI) {
